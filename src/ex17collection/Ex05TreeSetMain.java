...
- Set의 기본적인 특성은 동일하다.
-따라서 중복은 허용되지 않는다.
-단 인스턴스가 정렬되어 저장된다. 정령의 기준은 개발자가 직접 정의할 수 있다.
 */
...
class MyString implements Comparable<MyString> {
    ...
    /*
     * TreeSet<T>에 객체를 저장할 때 정렬을 위해 compareTo() 메서드를 오버라이딩한다.
     * 정렬의 기준은 단순하게 가나다와 같은 순서가 될 수도 있꼬, 문자열의 길이에
     * 따른 정렬이 될 수도 있다.
     * 아래의 코드는 오름차순(작은것 -> 큰것) 정렬일 때 사용한다.
     * 1과 -1을 바꾸면 내림차순이 된다.
     */
    @Override
...
public class Ex05TreeSetMain {
    ...
    public static void main(String[] args) {
        ...
        /*
         * Java에서 제공하는 기본클래스인 경우 별도의 오버라이딩 없어도
         * 정렬되어 저장된다.
         */
