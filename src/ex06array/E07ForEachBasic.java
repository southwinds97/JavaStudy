...
// 배열의 요소를 참조의 형태로 가져와서 사용한다. 따라서 참조의 목적으로만 사용해야 하며
// 값의 변경은 허용하지 않는다.
// 형식]
// for(자료형 참조변수 : 배열의 이름) {
//     실행문;
...
public class E07ForEachBasic {
    ...
    public static void main(String[] args) {
        ...
        // 더이상 참조할 요소가 없을 때 for문이 자동으로 종료된다.
        sum = 0;
        for (int e : arr) {
            System.out.print(e + " ");
            sum += e;
        ...
        // 1증가는 기존 배열에 적용되지 않으므로 기존 배열의 값이 그대로 출력된다.
        for (int c : arr) {
            System.out.print(c + " "); // 처음의 값이 그대로 출력됨
        }
        System.out.println();
        ...
        // 2차원 배열에서 하나의 행씩 접근해서 1차원배열을 가져온다.
        for (int[] a : twoDim) {
            // 앞에서 꺼낸 배열의 각 원소에 접근해서 출력한다.
            for (int b : a) {
                System.out.print(b + " ");
            }
            // 한 행 출력 후 줄바꿈 처리
