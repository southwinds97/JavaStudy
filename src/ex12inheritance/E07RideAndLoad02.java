...
public class E07RideAndLoad02 {
    ...
    public static void main(String[] args) {
        ...
        child.walk();
        // 자식쪽에 정의된 메서드로 오버로딩 정의되었다.
        child.walk(25);

        /*
         * 정적메서드는 클래스명을 통해 호출한다. static의 기본규칙을 따르고
         * 오버라이딩의 대상이 될 수 없다. 인스턴스 내부가 아닌 외부(Method영역)에
        ...
        /*
         * 오버라이딩 처리된 메서드이므로 참조변수와 상관없이 항상 자식쪽의
         * 메서드가 호출된다. 자식인스턴스가 생성될 떄 부모쪽의 메서드가
         * 가려지게 되므로 참조변수의 영향을 전혀 받지 않는다.
         */
        parent.excecise();
        parent.sleep();
        parent.printParent();
