...
오버로딩 : 메서드명은 같지만 매개변수의 갯수, 타입, 순서가 다른 경우를
...
class B extends A {
  ...
  // 오버로드로 정의된 메서드 부모와 매개변수의 갯수가 다름
...
class C extends B {
  ...
  /*
   * loadMethod()는 오버로딩된 메서드로 상위클래스에서 정의된 멤버메서드와
   * 이름만 동일할뿐 매개변수의 타입, 개수 등이 다르게 정의된다.
   * 따라서 다른 메서드이다.
   */
  ...
  /*
   * 따라서 C클래스로 인스턴스를 생성하면 오버라이딩 된 메서드는
   * 모두 가려지게 되어 C클래스에 정의한 1개만 남게 된다.
   * 오버로딩 된 메서드는 서로 다르므로 3개 모두 포함된다.
   */
}
...
public class E07RideAndLoad01 {
  ...
  public static void main(String[] arg) {
     ...
     */
    System.out.println("오버라이딩 처리");
    ref1.rideMethod();
    ref2.rideMethod();
    ref3.rideMethod();

    System.out.println("오버로딩 처리");
    /*
     ...
     */
    ref1.loadMethod();
    // ref1.loadMethod(1); 에러 발생
    // ref1.loadMethod(1.1); 에러 발생
     ...
     * 있기 떄문에 가능하고, 방식은 기본자료형과 동일하게 소괄호()를 이용하면 된다.
     */
    B refId2 = (B) refId1;
    C refId3 = (C) refId1;
