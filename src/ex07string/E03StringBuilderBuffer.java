 ...
 * 메모리에 저장한다. 기존 메모리가 소멸되고 새로운 메모리가 생성되는 낭비를
 * 막기 위해 문자열의 변경이 많은 경우라면 StringBuffer를 사용하는 것이 좋다.
...
public class E03StringBuilderBuffer {
    ...
    public static void main(String[] args) {
        ...
        // String은 변경이 있는 경우 기존 메모리를 소멸한 후 새로운 메모리를 할당
        str1 = str1 + "&Spring";
        if (str1 == str2) {
            System.out.println("연결후: 주소값 동일");
        } else {
            System.out.println("연결후: 주소값 다름"); // 출력
        }

        /*
         * StringBuffer는 문자열을 저장하기 위해 기본 메로리를 16으로 할당한다.
