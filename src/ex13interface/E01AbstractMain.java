...
-인스턴스 생성은 할 수 없지만 그 외 참조변수선언, 상속 등의 모든 기능을 사용할 수 있다.
 ...
 * 해당 클래스도 abstract로 선언해야 한다. 단, 이 경우 해당 클래스로도
 * 인스턴스를 생성할 수 없게 된다. 따라서 인스턴스 생성을 위해서는 부모의
 * 추상메서드를 자식쪽에서 오버라이딩을 필수로 해야 한다.
 */
class AbsClassChild2 extends AbsClass2 {
    /*
     * 오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의
     * 하는 것이므로 자식 클래스는 추상메서드를 포함하지 않는 상태가 된다.
     * 따라서 인스턴스를 생성할 수 있다.
...
public class E01AbstractMain {
    ...
    public static void main(String[] args) {
        ...
        // 추상클래스를 상속한 하위클래스는 인스턴스 생성이 가능하다.
        AbsClassChild1 absClassChild1 = new AbsClassChild1();

        /*
         * 추상크랠스는 인스턴스 생성은 할 수 없지만 참조변수로 사용할 수 있다.
         * 부모타입의 참조변수로 자식 인스턴스를 참조한다.
         */
        AbsClass2 absClass2 = new AbsClassChild2();
        /*
         * 부모쪽의 멤버메서드를 호출 할 수 있다. 하지만 오버라이딩 되었으므로
         * 자식쪽의 메서드가 동작하게 된다.
         */
        absClass2.absMethod(100);
        /*
         * 하지만 부모타입의 참조변수로 자식영ㅇ역에 접근은 불가능하므로,
