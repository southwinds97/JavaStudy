...
단, 부모클래스는 반드시 public으로 선언해야한다. 만약default로
선언하면 접근 자체가 불가능하므로 impoirt도 할수없게된다.
 */
...
class Seoul extends Korea {
  ...
  인스턴스를 생성해야한다. 그 후 자식 인스턴스가 메모리에 생성된다.
   */
  public Seoul(String name, String city) {
    super(name); // 부모생성자 호출
    this.city = city; //멤버변수 초기화
  }
  /*
  4개의 접근지정자를 통해 정의한 메서드를 호출한다. 단 부모 클래스가
  ...
  public void callMethod() {
    ...
    /*
    이와같이 직접 호출할 수 없는 메서드는 접근 가능한 멤버메서드를
    통해 간접 호출하면 된다.
     */
  }
}
