...
class Car {
     ...
     * 자료형을 생성하는 것이 class를 정의하는 목적이 된다.
     */

    /*
     * 초기화 메서드1 : 매개변수가 없는 형태로 항상 고정된 값으로만 인스턴스를
     * 초기화 할 수 있다. 따라서 확장성이 없는 형태이다.
     */
    void initialize() {
        // Car 클래스의 멤버변수를 초기화 한다.
        carModel = "람보르기니";
        /*
         * 자동차 소유주를 표현한 Human타입의 멤버변수는 아직 인스턴스가 생성되지 않은
         * 상태이므로, 초기화를 위해 인스턴스를 생성한 후 초기화 해야 한다.
         */
        owner = new Human();
        owner.name = "스티브로져스";
        owner.age = 30;
...
public class E03CarMain {
    ...
    public static void main(String[] args) {
        ...
        /*
         * 자동차 2 : 초기화 메서드를 호출하는 대신 멤버변수에 직접 접근하여
         * 인스턴스를 초기화한다. 이 경우 인스턴스를 생성할 때마다 동일한
        ...
        /*
         * 자동차3 : 매개변수가 있는 메서드를 사용하여 인스턴스를 생성한다.
         * 이 경우 매개변수를 통해 다양한 값을 전달할 수 있으므로 여러형태의
         * 인스턴스를 생성할 수 있다. 따라서 가장 효율적인 코드라 할 수 있다.
         */
        Car car3 = new Car();
         ...
         * 예외가 발생하게 된다. 즉 인스턴스는 초기화 이후에 사용해야 한다.
         */
        Car car4 = new Car();
        car4.showCarInfo();
    }
