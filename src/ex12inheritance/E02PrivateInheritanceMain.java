...
class Account {
  ...
  */
  ...
  않는다. 디폴트 생성자는 개발자가 생성자를 정의하지 않을때만
  컴파일러에 의해 자동으로 추가된다.
   */
  ...
  default라도 접근은 허용된다. 즉 protected를 지워도 상관없다.
   */
...
class SavingAccount extends Account {
  ...
  매개변수가 하나인 생성자가 유일하므로 만약 super(x)를 생략하면 에러가
  발생한다.
   */
  ...
  public void saveMoney(int money) {
    /*
    부모클래스에 정의된 멤버변수의 money는 private이므로 자식쪽에서는
    보이지 않으므로 접근할 수 없다. 따라서 접근 가능한 protected로 선언된
    메서드를 통해 간접적으로 사용한다.
     */
    depositMoney(money);
    // super.money += money;
...
public class E02PrivateInheritanceMain {
  ...
  public static void main(String[] args) {
    ...
    통해서도 monmey에 접근할 수 없다 현재 접근하려는 위치가 클래스
    외부이기 떄문이다. 즉 private 멤버는 무조건 클래스 내부에서만
    접근을 허용한다.
     */
