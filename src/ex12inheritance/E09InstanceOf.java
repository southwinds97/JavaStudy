...
public class E09InstanceOf {
    ...
    /*
     * - 메서드 정의시 부모타입의 매개변수를 설정하면 자식타입의 인스턴스를
     * 모두 받을 수 있다.
     * - Box타입으로 인수를 받게되면 자동형변환(업캐스팅)이 되므로 자식 클래스의
     * 멤버메서드를 호출할 수 없게 된다.
     * - 이 때 자식의 멤버메서드를 호출하기 위해서는 강제형변환(다운캐스팅)이 필요하다.
     * - instanceof 연산자를 통해 형변환이 가능한지 판단한 후 true를 반환하면
     * 다운캐스팅을 진행한 후 메서드를 호출할 수 있게 된다.
     */

    static void wrapBox(Box b) {
        /*
         * 세가지 인스턴스 주 어떤 것이 전달되더라도 Box타입으로 자동형변환
         * 되므로 해당 매개변수를 통해 호출할 수 잇는 메서드는 boxWrap()이 유일하다.
         * 따라서 다운캐스팅이 필요하다.
         */
    ...
    public static void main(String[] args) {
        ...
        /*
         * String 인스턴스는 Box와는 아무런 관계까 없으므로 인수로 전달할 수 없다.
         * 즉 상속관계가 성립하지 않는다.
         */
