...
//    실행문장;
// }
// - if문의 조건문은 반드시 비교식 또는 논리식이어야 한다.
...
public class E01If01 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    // 2가지 조건 모두 만족하므로 print문이 실행된다.
    if (num % 2 == 0 && num >= 10) {
      System.out.printf("num은 짝수이고 10보다 크거나 같다.%n", num);
    }
    if (true) {
      System.out.println("무조건 실행되는 if문입니다.");
    }
    // if(조건식); => 세미콜론은 문장의 끝을 나타내므로 조건식에 상관없이 무조건 실행된다. 따라서 사용에 주의해야 한다.
    if (num % 2 != 0)
      ;
    {
      System.out.printf("%d은 홀수입니다.", num);
    }
    // 조건 검사후 실행할 문장이 1개인 경우에는 중괄호 생략할 수 있다.
    // 단, 실행문이 2개 이상인 경우에는 한 문장만 실행되므로 반드시 중괄호를 사용해야 한다.
    if (num % 2 == 0)
      System.out.println("\n" + num + "은 짝수입니다.");
    if (num % 2 == 0) {
    ...
    // 아스키코드를 알고 있을 때의 구현방법. 문자 0과 9사이의 아스키코드값은 48~57에
    // 해당하므로 구간 내에 있는지 확인할 수 있다.
    ...
    // 아스키코드를 모를 때 구현방법으로 문자 '0'과 '9'는 메모리에 저장 시
    // 아스키코드로 변환되므로 아래와 같은 조건으로도 판단 가능하다.
