 ...
 * -반환값이 없다. 따라서 선언시 void를 사용하면 안된다.
...
public class PersonConstructor {
    ...
    String addr;
    /*
     * 개발자가 생성자를 직접 정의하지 않으면 아래와 같이 매개변수와 실행부가 없는
     * '디폴트 생성자'가 컴퍼일러에 의해 자동생성된다.
     * 지금까지 인스턴스 생성시 new 클래스명()과 같이 사용했던 문장이 바로 디폴트
     * 생성자를 호출하는 것이다.
     */
    ...
    public PersonConstructor(String name) {
         ...
         * 클래스 자신, 즉 멤버를 가리킨다. 따라서 좌측항은 멤버변수, 우측항은
         * 매개변수가 된다.
         */
        this.name = name;
        age = 1;
    ...
    public PersonConstructor(String name, int age) {
         ...
         * 단, 생성자 내에서만 사용할 수 있고, 일반 멤버메소드에서는 사용할 수 없다.
    ...
    // 멤버메소드 : 멤버변수를 초기화할 목적으로 정의한 메소드
    void initialize(String name, int age, String addr) {
        /*
         * 해당 코드는 생성자 내에서만 사용할 수 있다. 생서앚는 인스턴스 생성시
         * 딱 한번만 호출되고, 개발자가 마음대로 호출할 수 없다.
         * 하지만 멤버메소드는 개발자가 원할 때 언제든지 호출이 가능하므로 생성자와는
         * 특성이 다르다.
         * 따라서 멤버메서드에서는 생성자를 호출할 수 없다.
         */
        // this(name, age, "미상"); // 에러발생

        // 멤버변수를 구분을 위한 this는 멤버메서드에서도 사용할 수 있다.
        this.name = name;
        this.age = age;
        this.addr = addr;
    }
