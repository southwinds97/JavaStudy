...
/*
인터페이스(Interface)
-클래스가 객체의 설계도라면 인터페이스는 클래스 상속간의 설계도라고 할 수 있다.
-자바는 단일 상속을 원칙으로 하지만, 인터페이스를 통해 다중 상속을 구현할 수 있다.
-추상메서드와 동일하게 상속을 목적으로 제작되므로 구현받은 하위클래스에서는
...
/*
 * 인터페이스는 콤마를 이용해서 여러개의 인터페이스를 동시에 '구현'할 수 있다.
 * 또한 "상속"과 "구현"을 동시에 사용할 수도 있다.
 */
class MyClass extends SimpleClass implements Myinterface1, Myinterface2 {

    /*
     * 인터페이스에 동일한 이름의 추상메서드가 존재하는 경우 하나만 오버라이딩하면 된다.
     * 오버라이딩은 항상 자식쪽의 메서드가 호출되므로 아무런 문제도 발생되지 않는다.
     */
    @Override
