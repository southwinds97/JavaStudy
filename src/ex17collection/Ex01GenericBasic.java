...
// 오렌지 전용박스 : Orange 인스턴스만 저장할 수 있는 클래스
class OrangeBox {
    // 멤버변수 : 오렌지 인스턴스
    Orange item;
...
// 과일상자 : Object기반으로 모든 과일(인스턴스)을 저장할 수 있는 클래스
class FruitBox {
    // 멤버변수 : 모든 인스턴스를 저장 가능
    Object item;
...
public class Ex01GenericBasic {
    ...
    public static void main(String[] args) {
        ...
        orange1.showInfo();
        /*
         * 상자1은 오렌지 전용박스이므로 다른과일(즉 인스턴스)를 저장할 수 있따
         * 컴파일 에러가 발생하므로 실행자체가 불가능하다.
         * 즉 자료형에는 안전하지만, 구현에는 불편함이 있따. 다른 인스턴스를
         * 저장하기 위해서는 또 다른 Box클래스를 생성해야한다.
         */

        /*
         * 아래와 같이 Orange가 아닌 인스턴스를 저장하면 즉시 컴파일에러가
         * 발생하여 실행할 수 없는 코드가 된다. 즉 자료형에는 안전하다고 볼 수 있다.
         */
         ...
         */
        // Orange 인스턴스를 저장후 꺼내는 건 문제없이 실행된다.
