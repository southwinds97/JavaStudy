...
: X has a Y -> X가 Y를 소유(포함)하고 있다. 여기서 권총을 소유한
경찰을 표현

Has-A와 같이 소유의 관계를 상속으로 표현하면 클래스간의 강한 연결고리가 형성되어
권총이 없는 경찰을 표현하기 힘들어진다. 따라서 이 경우에는
...
class Police {
  ...
  public Police(int bNum, int hCuff) {
    //수갑은 정수값을 통해 보유여부를 설정
    handCuffs = hCuff;
    /*
    권총의 보유여부는 총알이 없는 상태라면 권총이 없는것으로 간주하여
    null로 초기화 한다. 참조변수가 null을 할당받았다는 것은 참조할
    인스턴스가 없는것을 의미한다. 즉 힙영역에 생성된 인스턴스가 아예
    없으므로 소유하지 않는것을 표현할 수 있다.
     */
    if(bNum <=0) {
      gun = null;
    }
