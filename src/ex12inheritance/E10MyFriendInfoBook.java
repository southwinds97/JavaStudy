...
class HighFriend extends Friend {
  ...
  /*
   * 오버라이딩1 : 고딩친구의 전체정보를 출력하기 위해 부모에서 정의된 메서드를
...
public class E10MyFriendInfoBook {
   ...
   * 정의한다. main 메서드에서 직접 호출하기 위해 static으로 선언되어있다.
   */
   ...
   * 구성보다는 프로그램의 전반적인 흐름에 대해서만 기술하는 것이 좋다.
  ...
  public static void main(String[] args) {
     ...
     * brack문은 반복문을 탈출시키는 기능이 있으므로 이와같은 무한루프에서
     * 자주 사용된다.
...
/*
 * 해당 프로그램에서 기능을 담당하는 클래스로 '핸들러' 혹은 '매니저'
 * 클래스라고 부른다.
 */
class FriendInfoHandler {
  ...
  /*
   * 배열에 저장된 연락처 정보를 카운트하기 위한 변수로, 추가할 때마다
   * ++(증가연산자)로 1씩 증가시킨다.
   */
  ...
  /*
   * 저장된 치눅의 정보를 출력하기 위한 멤버메서드
   * 1. 친구정보를 추가할 때 High 혹은 Univ 인스턴스를 배열에 저장한다.
   * 2. 이 떄 인스턴스는 Friend로 자동 형변환되어 저장된다.
   * 3. 정보 출력시 배열에 입력된 개체수만큼 반복하여 각 인자를 통해 정보를 출력한다.
   * 4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 되어있으므로
   * 참조변수의 영향없이 항상 자식클래스에 오버라이딩 된 메서드가 호출된다.
   * 5. 즉 저장된 인스턴스는 Friend 타입이지만 오버라이딩을 통해 별도의 형변환이
   * 필요하지않다. 항상 원하는 정보를 간단히 출력할 수 있다.
   */
  public void showAllData() {
  ...
  // 저장된 연락처의 간력정보(2가지)만 출력
  public void showSimpleData() {
    /*
     * 만약 부모클래스인 Frinend에 showBasicInfo()가 정의하지 않았다면.
     * 자식클래스에서는 오버라이딩을 할 수 없으므로 개별적인 메서드로
     * 정의해야한다.
     * 정보가 저장되는 배열은 Friend 타입이므로 자식인스턴스를 저장하면
     * 자동으로 형변환(업캐스팅)된다 이 때 자식의 정보를 출력하는
     * 메서드를 바로 호출할 수 없으므로 아래와 같이 일일이 확인 후 강제형변환 (다운캐스팅) 해야한다.
  ...
  public void searchInfo() {
    ...
    for (int i = 0; i < numOfFriends; i++) {
      /*
       * 배열의 각 인덱스에 저장된 인스턴스이 참조값을 통해 멤버변수에
       * 접근한다. 검색을 위해 입력한 이름과 비고해서 일치하는 경우에는
       * 정보를 출력한다. compartTo 대신 equals 를 사용해도 결과는 동일하다.
       */
  ...
  public void deleteInfo() {
    ...
    /*
     * 배열의 원소 중 삭제된 것의 인덱스 값을 저장하기 위한 변수로
     * 삭제한 후 빈자리를 채워넣기 위해 사용된다. 인덱스는 0부터
     * 시작이므로 -1을 초기값으로 지정한다.
     */
    int deleteIndex = -1;
    ...
    if (deleteIndex == -1) {
       ...
       * 차후 검색이나 출력시 NullPointException이 발생할 수 있다.
       */
