...
public class DeChild extends DeParent {
  ...
  /*
   * 자식클래스에서 확장한 메서드로 부모클래에서는 정의되어 있지 않다.
   */
  public void study() {
    /*
  ...
  /*
   * 오버라이딩 아님 : 부모클래스에서 private 멤버로 선언했으므로
  ...
  /*
   * 메서드 오버라이딩의 조건
   * 1. 부모클래스에서 선언될 때와 비교하여 접근범위가 동일하거나 그보다 넓으면 가능하다.
   * 2. 부모에서 default로 선언하면 자식에서는 default 혹은 그보다 넓은 public으로 선언하면
   * 오버라이딩이 성립된다.
   */
  @Override
  public String sleep() {
  ...
  /*
   * 메서드의 형태가 부모쪽과 다르므로 오버라이딩이 될 수 없다
  ...
  /*
   * static(정적) 메서드는 오버라이딩의 대상이 될 수 없다. 클래스
   * 외부(메서드영역)에 별도로 로드되므로 오버라이딩 되지 않고
